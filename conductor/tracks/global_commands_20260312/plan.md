...
## Phase 2: Refinement and Auto-completion [checkpoint: c042db2]
- [x] Task: Add Global Commands to Auto-completer. (dc7957c)
    - [x] Task: Write Tests: Verify `help` and `exit` appear in tab completion. (dc7957c)
    - [x] Task: Implement: Update the completer function in `App.Run`. (dc7957c)
- [x] Task: Ensure robust tokenization for global commands. (dc7957c)
    - [x] Task: Write Tests: Test `help <arg>` and `exit <arg>` behavior. (dc7957c)
    - [x] Task: Implement: Handle extra arguments gracefully (e.g., ignore them). (dc7957c)
- [x] Task: Conductor - User Manual Verification 'Phase 2: Refinement and Auto-completion' (c042db2)
